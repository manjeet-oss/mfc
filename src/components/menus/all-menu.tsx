...
export default function AllMenu() {
  ...
  if (loading) {
    return <div className="flex justify-center items-center h-full min-h-[400px]"><Loading /></div>;
  }

  return (
    <div className="menu flex flex-wrap justify-center gap-8 mb-16">
      {Object.entries(menu).map(([category, items]) => (
        <div key={category} className="border px-2 py-4 md:p-5 rounded-[10px] w-[97%] max-w-md">
                ...
                <div className="flex justify-between items-center">
                  ...
                  {cart[item.id] ? (
                    <div className="flex justify-center items-center h-9">
                      <button className="relative -mr-2 z-10 h-6 w-6 bg-[#fb6218] rounded-md flex items-center justify-center" onClick={() => handleDecreaseQuantity(item.id)}>
                        <Minus size={16}/>
                      </button>
                      <span className="text-lg p-1 bg-foreground text-background rounded w-12 font-semibold flex justify-center items-center">
                        {cart[item.id].quantity}
                      </span>
                      <button className="relative -ml-2 z-10 h-6 w-6 bg-[#fb6218] rounded-md flex items-center justify-center" onClick={() => handleIncreaseQuantity(item.id)}>
                        <Plus size={16}/>
                      </button>
                    </div>
                  ) : (
                    <Button icon={<Plus/>} className="add-button font-semibold" onClick={() => handleAddToCart(item.id)}>Add</Button>
                  )}
                </div>
